...
class TaskBatchDatabase:
    ...
    def save_task(self, task: Task) -> int:
        with sqlite3.connect(self.db_path) as conn:
            ...
            if task.id is None:
                ...
                """, (
                    task.name, task.db, task.debug_outdir,
                    json.dumps(task.gemini) if task.gemini else None,
                    json.dumps(task.local_files) if task.local_files else None,
                    task.logfile, task.min_area, task.min_size, task.no_safe_search,
                    task.num_images, task.outdir,
                    json.dumps(task.paths) if task.paths else None,
                    task.queries, task.randomize, task.threads, task.timestamp,
                    task.verbose, task.visible, task.wait_between_scroll,
                    task.wait_first_load, now, now
                ))
                ...
                """, (
                    ...
                    task.queries, task.randomize, task.threads, task.timestamp,
                    task.verbose, task.visible, task.wait_between_scroll,
                    task.wait_first_load, now, task.id
                ))
            conn.commit()
            return task.id
    ...
    def save_batch(self, batch: Batch) -> int:
        with sqlite3.connect(self.db_path) as conn:
            ...
            if batch.id is None:
                ...
                """, (
                    batch.name, batch.auto_timestamped_dir, batch.base_output_dir,
                    json.dumps(batch.parameter_overrides) if batch.parameter_overrides else None,
                    json.dumps(batch.environment_variables) if batch.environment_variables else None,
    ...
    def _row_to_task(self, row) -> Task:
        return Task(
            id=row[0], name=row[1], db=row[2], debug_outdir=row[3],
            gemini=json.loads(row[4]) if row[4] else None,
            local_files=json.loads(row[5]) if row[5] else None,
            logfile=row[6], min_area=row[7], min_size=row[8],
            no_safe_search=bool(row[9]), num_images=row[10], outdir=row[11],
            paths=json.loads(row[12]) if row[12] else None,
            queries=row[13], randomize=bool(row[14]), threads=row[15],
            timestamp=bool(row[16]), verbose=bool(row[17]), visible=bool(row[18]),
            wait_between_scroll=row[19], wait_first_load=row[20],
    ...
    def _row_to_batch(self, row) -> Batch:
        return Batch(
            id=row[0], name=row[1], auto_timestamped_dir=bool(row[2]),
